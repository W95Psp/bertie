...
pub fn hkdf_expand_label(
    ...
    if len >= 65536 {
        ...
        let lenb = Bytes::from_seq(&U16_to_be_bytes(U16(len as u16)));
...
pub fn derive_secret(
    ...
    hkdf_expand_label(ha, k, label, &Bytes::from_seq(tx), hash_len(ha))
}
...
pub fn derive_binder_key(ha: &HashAlgorithm, k: &Key) -> Result<MacKey, TLSError> {
    ...
    derive_secret(
        ha,
        &early_secret,
        &Bytes::from_seq(&LABEL_RES_BINDER),
        &hash_empty(ha)?,
    )
}
...
pub fn derive_aead_key_iv(
    ...
    let sender_write_key = hkdf_expand_label(
        ha,
        k,
        &Bytes::from_seq(&LABEL_KEY),
        &empty(),
        ae_key_len(ae),
    )?;
    let sender_write_iv =
        hkdf_expand_label(ha, k, &Bytes::from_seq(&LABEL_IV), &empty(), ae_iv_len(ae))?;
    Ok((
...
pub fn derive_0rtt_keys(
    ...
    let client_early_traffic_secret =
        derive_secret(ha, &early_secret, &Bytes::from_seq(&LABEL_C_E_TRAFFIC), tx)?;
    let early_exporter_master_secret =
        derive_secret(ha, &early_secret, &Bytes::from_seq(&LABEL_E_EXP_MASTER), tx)?;
...
//     let derived_secret = derive_secret(ha, &early_secret, &Seq::from_seq(&LABEL_DERIVED), &digest_emp)?;
...
//         derive_secret(ha, &handshake_secret, &Seq::from_seq(&LABEL_C_HS_TRAFFIC), tx)?;
//     //    println!("c h ts: {}", client_handshake_traffic_secret.to_hex());
//     let server_handshake_traffic_secret =
//         derive_secret(ha, &handshake_secret, &Seq::from_seq(&LABEL_S_HS_TRAFFIC), tx)?;
...
//     let master_secret_ = derive_secret(ha, &handshake_secret, &Seq::from_seq(&LABEL_DERIVED), &digest_emp)?;
...
//         derive_secret(ha, master_secret, &Seq::from_seq(&LABEL_C_AP_TRAFFIC), tx)?;
//     let server_application_traffic_secret_0 =
//         derive_secret(ha, master_secret, &Seq::from_seq(&LABEL_S_AP_TRAFFIC), tx)?;
...
//     let exporter_master_secret = derive_secret(ha, master_secret, &Seq::from_seq(&LABEL_EXP_MASTER), tx)?;
//     Ok((
//         client_write_key_iv,
//         server_write_key_iv,
...
//     derive_secret(ha, master_secret, &Seq::from_seq(&LABEL_RES_MASTER), tx)
// }
...
//         verify(&sig_alg(&algs), &pk, &Seq::from_seq(&sigval), &sig)?;
